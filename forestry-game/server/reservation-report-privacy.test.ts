...
it('shares settled reservation results while keeping crew/truck orders private',()=>{
 ...
 try{
  ...
  act(tokens.production,'plan',{crews:{...store.view(owner.id,tokens.production).game.plan.crews,C1:[{stand:'Q01',hours:40}]},reservations:[{id:'r1',stand:'Q01',product:'soft-saw',mill:'M1',volume:40,market:'ordinary'}]});
  ...
  for(const token of Object.values(tokens)){
   ...
   expect(h.reservationFulfillment).toEqual({r1:40});expect(h.plan.crews).toEqual({});expect(h.plan.trucks).toEqual({});expect(()=>parseGame(JSON.stringify(g))).not.toThrow();
  }
