...
import {createGame,advance,purchase,sum,stockAt,draftPlan} from './engine';
...
function overlay(source:RegionDefinition){
 ...
 // This overlay deliberately exercises the legacy deferred-payment mechanism.
 // The default BC tenure/stumpage model has its own authorization playthroughs.
 delete r.bcTenure;
 delete r.bcMarket;
 ...
 return {r:subdividedScenario(r,7),standIds:stands.map(s=>s.id),p};
}
describe('complete daily optional-mechanic campaigns on regional graphs',()=>{
 it.each([['Québec',quebec],['Prince George',princeGeorge]] as const)('%s: daily royalties, reciprocal loads, processing and byproduct transfers survive every reload',(_name,source)=>{
  ...
  let totalHarvest=0,totalDelivered=0;
  ...
  while(g.week<=r.weeks){
   ...
   g=advance(g);
   totalHarvest+=sum(g.history.at(-1)!.harvested);totalDelivered+=sum(g.history.at(-1)!.delivered);
   ...
   for(const state of Object.values(g.processing??{}))expect(state.received).toBeCloseTo(sum(state.input)+sum(state.output)+sum(state.sold)+sum(state.transferred??{})+state.residue,5);
  ...
  expect(totalHarvest).toBeGreaterThan(100);expect(totalDelivered).toBeGreaterThan(100);
  expect(g.history).toHaveLength(source.weeks*7);expect(g.reciprocal!.pair.movedM3,JSON.stringify(g.history[0].messages)+' '+JSON.stringify(g.history[0].harvested)).toBeGreaterThan(0);expect(g.history.flatMap(h=>h.facilityTransfers??[]).length).toBeGreaterThan(0);expect(g.history.flatMap(h=>h.ledger).some(e=>e.category==='royalty'&&e.amount<0)).toBe(true);expect(g.history.flatMap(h=>h.ledger).some(e=>e.category==='finished-sales'&&e.amount>0)).toBe(true);
 },300000);
});

describe.runIf(process.env.FOREST_SLOW_INTEGRATION==='1')('unmodified regional daily scenarios',()=>{
 it.each([['Québec',quebec],['Prince George',princeGeorge]] as const)('%s completes all 84 daily decisions with actual weather and adaptive operations',(_name,source)=>{
  const r=subdividedScenario(source,7),original=r.stands.reduce((n,s)=>n+s.volume,0);let g=createGame(r),harvest=0,delivered=0;
  while(g.week<=r.weeks){
   g=draftPlan(g,{commitmentAware:true,salesPolicy:'penalty-aware'});g=advance(g);
   const report=g.history.at(-1)!;harvest+=sum(report.harvested);delivered+=sum(report.delivered);
   expect(g.stands.reduce((n,s)=>n+s.remaining+sum(stockAt(g,s.id)),0)+g.history.reduce((n,h)=>n+sum(h.delivered)+h.waste,0)).toBeCloseTo(original,3);
   expect(lotProfitability(g).reconciliationDifference).toBeCloseTo(0,3);
   for(const truck of r.trucks)expect(report.truckHours[truck.id]).toBeLessThanOrEqual(truck.hours+1e-6);
   for(const crew of r.crews)expect(report.crewHours[crew.id]).toBeLessThanOrEqual(crew.hours+1e-6);
   const reloaded=parseGame(serializeGame(g));expect(reloaded.cash).toBe(g.cash);expect(reloaded.history).toEqual(JSON.parse(JSON.stringify(g.history)));g=reloaded;
  }
  expect(g.history).toHaveLength(84);expect(harvest).toBeGreaterThan(100);expect(delivered).toBeGreaterThan(100);
 },300000);
});

it('preserves zero-value ledger economics when JSON canonicalizes negative zero',()=>{
 const r=subdividedScenario(quebec,7);let g=createGame(r);while(g.week<=r.weeks)g=advance(g);
 expect(g.history.at(-1)!.ledger.some(e=>Object.is(e.amount,-0))).toBe(true);
 const reloaded=parseGame(serializeGame(g));expect(reloaded.history).toEqual(JSON.parse(JSON.stringify(g.history)));expect(reloaded.cash).toBe(g.cash);
});
