...
export default function PreSeasonDesk({game,onChange,onMobilize,role}:{game:Game;onChange?:(g:Game)=>void;onMobilize?:(kind:'crew'|'truck',id:string,to:string)=>void;role?:string}){
 ...
 <details><summary>{tr("Positioning record ·")} {game.mobilization?.moves.length??0} {tr("moves")}</summary>{game.mobilization?.moves.map((m,i)=><p key={i}>{m.resource}: {m.from} → {m.to} · {m.hours.toFixed(1)}h · {game.region.currency} {m.cost.toFixed(2)}</p>)}</details></section>;
}
