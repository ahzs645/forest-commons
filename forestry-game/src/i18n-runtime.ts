...
const patterns: [RegExp,(...parts:string[])=>string][] = [
 ...
 [/^(.+): no haul route to a buyer is forecast open before wood cut this turn spoils \(next open turn: (none this season|\d+)\)\. Unhauled wood becomes waste\.$/,(_,id,next)=>`${id} : aucune route de transport vers un acheteur n’est prévue ouverte avant que le bois coupé ce tour-ci ne se gâte (prochain tour ouvert : ${next==='none this season'?'aucun cette saison':next}). Le bois non transporté devient un rebut.`],
 [/^(.+): no haul route to a buyer is forecast open until turn (\d+)\. Pulp cut this turn becomes waste first, and sawlogs become pulp\.$/,(_,id,next)=>`${id} : aucune route de transport vers un acheteur n’est prévue ouverte avant le tour ${next}. La pâte coupée ce tour-ci devient d’abord un rebut, et les billes de sciage deviennent de la pâte.`],
 [/^(.+): sawlogs cut this turn are forecast to become pulp before a haul route opens \(next open turn: (none this season|\d+)\)\.$/,(_,id,next)=>`${id} : les billes de sciage coupées ce tour-ci devraient devenir de la pâte avant l’ouverture d’une route de transport (prochain tour ouvert : ${next==='none this season'?'aucun cette saison':next}).`],
