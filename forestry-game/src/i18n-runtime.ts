...
const patterns: [RegExp,(...parts:string[])=>string][] = [
 ...
 [/^(.+): carried (.+) m³ partner cargo on (.+); additional handling and travel consumed (.+) h\.$/,(_,truck,volume,id,hours)=>`${truck} : ${volume} m³ de fret partenaire transportés pour ${id}; la manutention et le trajet supplémentaires ont consommé ${hours} h.`],
 [/^Won (.+) for (.+)\. Available next week; refusal window lasts one week\.$/,(_,id,bid)=>`${id} remporté pour ${bid}. Disponible au prochain tour; le refus reste possible pendant une semaine physique.`],
 [/^(.+): insufficient cash and credit at settlement\.$/,(_,id)=>`${id} : trésorerie et crédit insuffisants au règlement.`],
 [/^(.+): rival bid (.+) won\.$/,(_,id,bid)=>`${id} : l’offre concurrente de ${bid} a remporté le lot.`],
 ...
 [/^Won (.+); rival (.+)$/,(_,id,bid)=>`${id} remporté; concurrent ${bid}`],
